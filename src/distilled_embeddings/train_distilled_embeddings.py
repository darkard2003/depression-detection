...
DATA_DIR = "data_processed/processed_embeddings"
TEACHER_DIR = "data_processed/processed_chi2"
OUTPUT_DIR = "outputs/distilled_embeddings"
PROJECT_NAME = "reddit_mlp_distilled_embeddings"
ALPHA = 0.1
SEED = 42

def build_model(hp):
    model = keras.Sequential()
    # Input layer for 384-dimensional sentence embeddings
...
def main():
    ...
    print("Loading pre-computed embeddings and targets...")
    X = np.load(os.path.join(DATA_DIR, "X_embeddings.npy"))
    y = np.load(os.path.join(TEACHER_DIR, "y.npy"))
    y_teacher = np.load(os.path.join(TEACHER_DIR, "y_teacher_soft.npy"))
    ...
    tuner = kt.Hyperband(
        ...
        max_epochs=20,
        factor=3,
        directory='outputs/distilled_embeddings/tuning',
