...
STUDENT_MODEL_PATH = "outputs/distilled_hybrid_gated/reddit_mlp_distilled_hybrid_gated/best_model.keras"
TEACHER_MODEL_NAME = "TRT1000/depression-detection-model"
# Note: Reuse the fitted TF-IDF vectorizer from the standard hybrid project folder
...
def main():
    print("=" * 60)
    print("Evaluating Gated Hybrid Student Model Fidelity to Teacher")
    print("=" * 60)
    ...
    # NOTE: safe_mode=False is required to deserialize Python lambda layers.
    # Since we built and saved this model locally, this is safe.
    # Production alternative (Option 2): Define a custom Keras Layer subclass registered
    # with @keras.saving.register_keras_serializable to support safe_mode=True.
    student_model = tf.keras.models.load_model(STUDENT_MODEL_PATH, safe_mode=False)
