...
def train_variant(variant_name, X, y_hard, y_soft, output_subdir):
    print("\n" + "=" * 50)
    print(f"Training Distilled Lite: {variant_name}")
    print("=" * 50)
    ...
    # Train directly on the soft targets
    history = model.fit(
        X_train, 
        y_soft_train, 
        epochs=100, 
...
def main():
    ...
    if not (os.path.exists(X_path) and os.path.exists(y_hard_path)):
        print("Error: Lite features not generated yet!")
        sys.exit(1)
    ...
    if os.path.exists(y_soft_orig_path):
        ...
        train_variant(
            ...
            y_hard=y_hard,
            y_soft=y_soft_orig,
            output_subdir=os.path.join(OUTPUT_DIR, "reddit_mlp_distilled_lite_combined_orig")
        )
    else:
    ...
    if os.path.exists(y_soft_ft_path):
        ...
        train_variant(
            ...
            y_hard=y_hard,
            y_soft=y_soft_ft,
            output_subdir=os.path.join(OUTPUT_DIR, "reddit_mlp_distilled_lite_combined_ft")
        )
    else:
